...
private fun GuiGraphicsExtractor.renderRoomText(room: DungeonRoom) {
    ...
    val maxSecrets = scannedRoom.data?.secrets ?: room.data?.secrets ?: 0
    val isGreen = room.checkmark == MapCheckmark.GREEN
    val roomNameKey = scannedRoom.name ?: scannedRoom.data?.name ?: room.name ?: room.data?.name
    val secretsFound = if (isGreen) maxSecrets else (roomNameKey?.let { DungeonMap.roomSecrets[it] } ?: 0)

    val textColor = when {
        isGreen -> Colors.MINECRAFT_GREEN
        room.checkmark == MapCheckmark.WHITE -> Colors.WHITE
        room.checkmark == MapCheckmark.RED -> Colors.MINECRAFT_RED
        else -> Color(100, 100, 100)
    }.rgba

    val roomName = scannedRoom.name
    val nameLines = roomName?.split(" ") ?: emptyList()

    val (lines, finalTextColor) = when (DungeonMap.textMode) {
        1 -> {
            if (nameLines.isEmpty()) return
            nameLines to textColor
        }
        2 -> {
            if (maxSecrets == 0) {
                listOf("0/0") to Colors.MINECRAFT_GREEN.rgba
            } else {
                listOf("$secretsFound/$maxSecrets") to textColor
            }
        }
        else -> { // "Both" or 0
            if (maxSecrets == 0) {
                if (nameLines.isEmpty()) return
                nameLines to Colors.MINECRAFT_GREEN.rgba
            } else {
                if (nameLines.isEmpty()) return
                (nameLines + "$secretsFound/$maxSecrets") to textColor
            }
        }
    }
    ...
    for ((i, line) in lines.withIndex()) {
        ...
        centeredText(mc.font, line, 0, -fontH / 2, finalTextColor)
        pose().popMatrix()
    }
}

private fun GuiGraphicsExtractor.renderPlayers() {
    val showNames = mc.player?.mainHandItem?.itemId?.equalsOneOf("INFINITE_SPIRIT_LEAP", "SPIRIT_LEAP") == true || DungeonMap.showNameAlways
    ...
    for (player in DungeonUtils.dungeonTeammatesNoSelf) {
        ...
        player.playerSkin?.let { skin ->
            val baseSize = 8
            val scaledSize = (baseSize * DungeonMap.playerHeadSize).toInt()

            if (DungeonMap.playerHeadBackgroundSize != 0 && scaledSize > 0) {
                val bgSize = scaledSize / 2 + DungeonMap.playerHeadBackgroundSize
                fill(-bgSize, -bgSize, bgSize, bgSize, player.clazz.color.rgba)
            }

            if (scaledSize > 0) {
                val half = scaledSize / 2
                PlayerFaceExtractor.extractRenderState(this, skin, -half, -half, scaledSize)
            }
        }
